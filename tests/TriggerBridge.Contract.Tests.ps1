...
Assert-Match $bridge "(?s)definition:\s*\{.*parameters:\s*\{\s*protectedTriggerUrl:\s*\{\s*type:\s*'SecureString'" 'The workflow definition must declare the protected trigger URL as SecureString.'
Assert-Match $bridge "body:\s*'@triggerBody\(\)'" 'The bridge must forward the original request body.'
Assert-Match $bridge "uri:\s*'@parameters\(\\'protectedTriggerUrl\\'\)'" 'The HTTP action URI must reference the workflow SecureString parameter.'
Assert-NotMatch $bridge '(?m)^\s*uri:\s*protectedTriggerUrl\s*$' 'The HTTP action must not embed the ARM deployment parameter directly.'
...
Assert-Match $bridge "operationOptions:\s*'DisableAsyncPattern'" 'The bridge must return the SRE Agent HTTP 202 without polling its Location header.'
Assert-Match $bridge "(?s)runtimeConfiguration:\s*\{\s*secureData:\s*\{\s*properties:\s*\[\s*'inputs'\s*\]" 'The HTTP action inputs must be protected in run history.'
Assert-Match $bridge "(?ms)^\s{4}parameters:\s*\{\s*protectedTriggerUrl:\s*\{\s*value:\s*protectedTriggerUrl" 'The workflow resource parameter must be wired from the secure Bicep parameter.'
...
Assert-Match $configuration '(?s)\$existingRepositoryBranchMatchesDesired = \[string\]::IsNullOrWhiteSpace\(\$existingRepositoryBranch\).*?-or \$existingRepositoryBranch -eq ''main''.*?-and \$existingRepositoryBranchMatchesDesired' 'Repository validation must accept a null provider-default branch while rejecting explicit non-main branches.'
